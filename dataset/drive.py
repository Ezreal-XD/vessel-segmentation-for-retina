...
class DriveValDataSet(data.Dataset):
...
class DriveTestDataSet(data.Dataset):
...
"""
"""
def gen_whole_list_txt(t_path="training/images",
                       m_path="training/1st_manual",
                       z_path="test/images"):
    lst = []
    print("./drive/"+t_path)
    for name in os.listdir("./drive/"+t_path):
        img_path = t_path + "/" + name
        lab_path = m_path + "/" + name.replace('tif', 'gif')
        lab_path = lab_path.replace('training.', 'manual1.')
        # print(img_path+"\n")
        # print(img_path + " " + lab_path+"\n")
        lst.append(img_path + " " + lab_path)
    test_lst = []
    for name in os.listdir("./drive/"+z_path):
        img_path = z_path + "/" + name
        # print(img_path+"\n")
        # print(img_path + " " + lab_path+"\n")
        test_lst.append(img_path)
    ...
    print(len(test_lst))
    m = 0.75
    with open('./drive/drive_train.txt', mode='w') as f:
        for x in lst[: int(m*len_lst)]:
            f.write(x + "\n")
    with open('./drive/drive_val.txt', mode='w') as f:
        for x in lst[int(m * len_lst):]:
            f.write(x + "\n")
    with open('./drive/drive_test.txt', mode='w') as f:
        for x in test_lst:
            f.write(x + "\n")
    with open('./drive/drive_trainval.txt', mode='w') as f:
        for x in lst:
            f.write(x + "\n")
...
def gen_test_list_txt(z_path="img_train"):
    ...
    print(len(test_lst))
    m = 0.99
    with open('./drive/drive_testB_list.txt', mode='w') as f:
        for x in test_lst:
            f.write(x + "\n")
...
def co_transforms(img, msk):
    img = TF.resize(img, [360, 480])
    msk = TF.resize(msk, [360, 480])
