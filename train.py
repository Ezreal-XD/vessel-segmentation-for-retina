...
from utils.metric.metric import get_iou, get_dice
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default="drive", help="dataset: cityscapes or camvid")
    parser.add_argument('--input_size', type=str, default="565,584", help="input size of model")
    parser.add_argument('--num_workers', type=int, default=4, help=" the number of parallel threads")
    parser.add_argument('--classes', type=int, default=2,
                        help="the number of classes in the dataset. 19 and 11 for cityscapes and camvid, respectively")
    parser.add_argument('--ignore_label', type=int, default=255, help="background class")
    ...
    parser.add_argument('--none_gt', type=bool, default=False, help="True for cityscapes")
    # loss
...
def train_model(args):
    ...
    print("the number of parameters: %d ==> %.2f M" % (total_parameters, (total_parameters / 1e6)))

    # load data and data augmentation
    datas, trainLoader, valLoader = build_dataset_train(args.dataset, args.classes, input_size,
                                                        args.batch_size, args.train_type,
                                                        False, False, args.num_workers)
    ...
    if args.dataset == 'camvid':
        criteria = CrossEntropyLoss2d(weight=weight, ignore_label=args.ignore_label)
        print(weight)
    elif args.dataset == 'camvid' and args.use_label_smoothing:
        criteria = CrossEntropyLoss2dLabelSmooth(weight=weight, ignore_label=args.ignore_label)

    elif args.dataset == 'cityscapes' and args.use_ohem:
        min_kept = int(args.batch_size // len(args.gpus) * h * w // 16)
        criteria = ProbOhemCrossEntropy2d(use_weight=True, ignore_label=args.ignore_label, thresh=0.7,
                                          min_kept=min_kept)
        ...
        criteria = CrossEntropyLoss2d(weight=weight, ignore_label=args.ignore_label)
    elif args.dataset == 'drive' and args.use_ohem:
        min_kept = int(args.batch_size // len(args.gpus) * h * w // 16)
        criteria = ProbOhemCrossEntropy2d(use_weight=True, ignore_label=args.ignore_label, thresh=0.7,
                                          min_kept=min_kept)
    elif args.dataset == 'drive' and args.use_label_smoothing:
        criteria = CrossEntropyLoss2dLabelSmooth(weight=weight, ignore_label=args.ignore_label)
    elif args.dataset == 'drive' and args.use_lovaszsoftmax:
        criteria = LovaszSoftmax(ignore_index=args.ignore_label)
    elif args.dataset == 'drive' and args.use_focal:
        criteria = FocalLoss2d(weight=weight, ignore_index=args.ignore_label)
    else:
        criteria = CrossEntropyLoss2d(weight=weight, ignore_label=args.ignore_label)
    ...
    if os.path.isfile(logFileLoc):
        ...
        logger.write("Parameters: %s Seed: %s" % (str(total_parameters), GLOBAL_SEED))
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if epoch % 2 == 0 or epoch == (args.max_epochs - 1):
            ...
            mIOU_val, per_class_iu, mean_dice = val(args, valLoader, model)
            mIOU_val_list.append(mIOU_val)
            # record train information
            logger.write("\n%d\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.7f" % (epoch, lossTr, mIOU_val, mean_dice, lr))
            logger.flush()
            print("Epoch : " + str(epoch) + ' Details')
            print("Epoch No.: %d\tTrain Loss = %.4f\t mIOU(val) = %.4f\t mDice(val) = %.4f\t lr= %.6f\n" % (
            epoch, lossTr, mIOU_val, mean_dice, lr))
        else:
            # record train information
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        dice = get_dice(output, gt)
        dice_list.append(dice)

    meanIoU, per_class_iu = get_iou(data_list, args.classes)
    mean_dice = sum(dice_list) / total_batches
    return meanIoU, per_class_iu, mean_dice
...
def main():
    ...
    if args.dataset == 'cityscapes':
        ...
        args.ignore_label = 11
    elif args.dataset == 'drive':
        args.classes = 2
        args.input_size = '565,584'
        args.ignore_label = 2
    else:
        raise NotImplementedError("please register here first!")
